...
#     def __init__(self, config):
#         self.config = config
#         self.modulation = config['system'].get('modulation')
#         self.baud_rate = float(config['system']['baud_rate'])
#         self.os_factor = int(config['system']['os_factor'])  # Analog OS (e.g., 8)
#
#         # Tx YAML Impairments (Using default fallbacks for safety)
#         self.tx_cfg = config.get('tx')
#         self.clip_papr = self.tx_cfg.get('dac', {}).get('clip_papr_db_max')
#         self.enob = self.tx_cfg.get('dac', {}).get('physical_bits')
#         self.iq_gain_imb = self.tx_cfg.get('iq_imbalance', {}).get('gain_db_sigma_max')
#         self.iq_phase_imb = self.tx_cfg.get('iq_imbalance', {}).get('phase_deg_sigma_max')
#
#         # Pre-calculate 60GHz Analog Tx Filter (at os_factor=8)
#         # cutoff_ratio = 0.6 * 100Gbd = 60GHz
#         self.tx_afe_taps = generate_bessel_taps(self.os_factor, cutoff_ratio=0.6, span=16)
#
#     def transmit_frame(self, batch_size, payload_bits):
#         """Executes the full pipeline for a batched row tensor."""
#         _, _, bits_per_ch = get_wifi_qam_lut(self.modulation)
...
#         Q_oqam = np.concatenate((padding, Q_2x), axis=1)
#
#         # C. PAPR Clipping (TODO reverify)
#         clip_linear = 10 ** (self.clip_papr / 20.0)
#         I_clipped = np.clip(I_oqam, -clip_linear, clip_linear)
#         Q_clipped = np.clip(Q_oqam, -clip_linear, clip_linear)
#
#         # D. Quantization (Mathematical rigid grid)
#         levels = 2 ** self.enob
#
#         # Map to 0-1, multiply by levels, floor, map back
#         def quantize(x):
#             x_norm = (x + clip_linear) / (2 * clip_linear)
#             x_quant = np.floor(x_norm * (levels - 1) + 0.5)
#             # Inject static INL/DNL here if desired by adding static noise to x_quant
#             return (x_quant / (levels - 1)) * (2 * clip_linear) - clip_linear
#
#         I_quantized = quantize(I_clipped)
#         Q_quantized = quantize(Q_clipped)
#
#         return I_quantized, Q_quantized
#
#     def _step3_analog_domain(self, I_2x, Q_2x):
#         """200GSa/s to 800GSa/s (4x OS), Filtering, and PA Non-linearities."""
#         # A. Continuous Time Projection (800 GSa/s)
#         # We need an overall os_factor of 8. Since we are at 2x, repeat by 4.
#         analog_os = self.os_factor // 2
#         I_8x = np.repeat(I_2x, analog_os, axis=1)
#         Q_8x = np.repeat(Q_2x, analog_os, axis=1)
#
#         # B. Tx AFE Linear Low-Pass Filtering
#         I_filt = fft_convolve_same(I_8x, self.tx_afe_taps)
#         Q_filt = fft_convolve_same(Q_8x, self.tx_afe_taps)
#
#         # C. RF Driver Memory Polynomial (MPM)
#         # y(n) = x(n) - c1*x(n)|x(n)|^2 - c2*x(n-1)|x(n-1)|^2
#
#         # Instantaneous power
#         mag_sq = I_filt ** 2 + Q_filt ** 2
#
#         # 1-sample delayed power (Memory effect at 800 GSa/s = 1.25ps memory)
#         I_delay = np.roll(I_filt, shift=1, axis=1)
#         Q_delay = np.roll(Q_filt, shift=1, axis=1)
#         mag_sq_delay = np.roll(mag_sq, shift=1, axis=1)
#
#         # Fix the first element of the roll to avoid wrap-around artifacts
#         I_delay[:, 0] = I_filt[:, 0]
#         Q_delay[:, 0] = Q_filt[:, 0]
#         mag_sq_delay[:, 0] = mag_sq[:, 0]
#
#         # C. RF Driver Memory Polynomial (Simplified 3rd order compression)
#         # Uses vector magnitude to compress the outer constellation points
#         mag_sq = I_filt ** 2 + Q_filt ** 2
#         compression_factor = 0.05  # TODO : Extract from yaml: rf_driver non_linear_coefficients
#         I_nl = I_filt - compression_factor * I_filt * mag_sq
#         Q_nl = Q_filt - compression_factor * Q_filt * mag_sq
#
#         return I_nl, Q_nl
#
#     def _step4_modulator_physics(self, I_nl, Q_nl):
#         """Apply Analog I/Q Imbalances and Electro-Optic Modulator sine curve."""
#         # A. I/Q Imbalance (Gain and Phase applied in continuous domain)
#         gain_lin = 10 ** (self.iq_gain_imb / 20.0)
#         phase_rad = np.radians(self.iq_phase_imb)
#
#         # Gain mismatch (I is slightly larger, Q is slightly smaller)
#         I_imb = I_nl * np.sqrt(gain_lin)
#
...
#
#         # B. Mach-Zehnder Modulator Transfer Function
#         # Maps electrical voltage to optical field via sin(V / Vpi * pi/2)
#         vpp_ratio = 0.8  # TODO:  Extract from yaml modulator vpp_to_vpi_ratio
#         I_opt = np.sin(I_imb * vpp_ratio * (np.pi / 2))
#         Q_opt = np.sin(Q_imb * vpp_ratio * (np.pi / 2))
#
#         return I_opt + 1j * Q_opt
#
#
# class TransmitterQam16:
#     def __init__(self, config):
#         self.config = config
#         self.baud_rate = float(config['system']['baud_rate'])
...
#
#     def generate_signal(self, os_factor, N, B=1, normalize=True):
#         """
#         Generate batched Offset-QAM16 symbols with proper zero-padding
#         to preserve trailing Q-channel data for pulse-shaping filters.
#
#         Parameters:
#         os_factor (int): Oversampling factor (samples per symbol)
#         N (int)         : Number of symbols per batch item
#         B (int)         : Batch size (first dimension)
#         normalize (bool): If True, normalizes average power to 1
#
#         Returns:
#         signal (ndarray)     : Complex OQAM16 signal of shape (B, N * os_factor + os_factor // 2)
#         i_bit_pairs (ndarray): Transmitted I bit-pair indices of shape (B, N)
#         q_bit_pairs (ndarray): Transmitted Q bit-pair indices of shape (B, N)
#         """
#         # Wi-Fi 16-QAM Gray mapping table for a 2-bit pair:
#         # 00 (0) -> -3
#         # 01 (1) -> -1
#         # 10 (2) -> +3
#         # 11 (3) -> +1
...
#
#         # Generate random 2-bit integer values for the entire batch matrix
#         i_bit_pairs = np.random.randint(0, 4, size=(B, N))
#         q_bit_pairs = np.random.randint(0, 4, size=(B, N))
#
#         # Map bit pairs to standard constellation amplitudes
#         I_symbols = mapping[i_bit_pairs]  # Shape: (B, N)
#         Q_symbols = mapping[q_bit_pairs]  # Shape: (B, N)
#
#         # Apply oversampling via sample repetition (Rectangular pulse shaping)
#         # This stretches the symbol axis (axis=1) from N to (N * os_factor)
#         I_oversampled = np.repeat(I_symbols, os_factor, axis=1)  # Shape: (B, N * os_factor)
#         Q_oversampled = np.repeat(Q_symbols, os_factor, axis=1)  # Shape: (B, N * os_factor)
#
#         # 4. Implement Offset-QAM: Delay the Q channel by T/2
#         # For an os_factor of 8, this translates to an exact shift of 4 samples.
#         offset = os_factor // 2
#         padding = np.zeros((B, offset))  # Shape: (B, offset)
#
#         # 5. Apply the Offset:
#         # Prepend zeros to Q (delaying it), append zeros to I (matching the length)
#         I_padded = np.concatenate((I_oversampled, padding), axis=1)  # Shape: (B, N * os_factor + offset)
#         Q_padded = np.concatenate((padding, Q_oversampled), axis=1)  # Shape: (B, N * os_factor + offset)
#
#         # 6. Combine into complex OQAM16 signal
#         tx_signal = I_padded + 1j * Q_padded
#
#         # 7. Apply Wi-Fi standard normalization factor
#         if normalize:
#             K_mod = 1 / np.sqrt(10)
#             tx_signal *= K_mod
#
#         # Design the Image Rejection Low-Pass Filter.
#         # Apply physical hardware roll-off (4th-order Bessel-Thomson) - this is maximally
#         # flat group delay; given the offset Q.
...
#
#         filtered_signal = fft_convolve_same(tx_signal, taps)
#
#         if False:
#             import matplotlib.pyplot as plt
#
#             # Plot the first 8 symbols (64 samples) of the first batch item
#             N_samples = 100
#             plt.figure(figsize=(10, 4))
#             plt.plot(np.real(tx_signal[0, :N_samples * os_factor]), 'r-', label='Re-Ideal OQAM16 (Stair-steps)',
#                      alpha=0.7,
#                      lw=2)
#             plt.plot(np.imag(tx_signal[0, :N_samples * os_factor]), 'g-', label='Im-Ideal OQAM16 (Stair-steps)',
#                      alpha=0.7,
#                      lw=2)
#
#             plt.plot(np.real(filtered_signal[0, :N_samples * os_factor]), 'm-', label='Re-Filtered Output (Smooth)',
#                      lw=2)
#             plt.plot(np.imag(filtered_signal[0, :N_samples * os_factor]), 'b-', label='Im-Filtered Output (Smooth)',
#                      lw=2)
#
#             plt.title("OQAM16 Transmitter Filtering (In-Phase Component)")
...
#
#         return tx_signal, filtered_signal, i_bit_pairs, q_bit_pairs
